  ...
  return (
    <Sidebar collapsible="icon" className="border-r border-purple-200/50">
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton size="lg" className="data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground">
              <div className="flex aspect-square size-8 items-center justify-center rounded-lg bg-gradient-to-br from-purple-500 to-orange-500 text-sidebar-primary-foreground">
                <Sparkles className="size-4 text-white" />
              </div>
              <div className="grid flex-1 text-left text-sm leading-tight">
                <span className="truncate font-semibold">Dev Campfire</span>
                <span className="truncate text-xs text-muted-foreground">Developer Community</span>
              </div>
            </SidebarMenuButton>
          </SidebarMenuItem>
        </SidebarMenu>
      ...
      <SidebarContent>
              ...
              {mainNavItems.map((item) => (
                <SidebarMenuItem key={item.id}>
                  <SidebarMenuButton
                    ...
                    className="data-[active=true]:bg-gradient-to-r data-[active=true]:from-purple-500 data-[active=true]:to-purple-600 data-[active=true]:text-white"
                  >
        ...
        {/* User Languages (when expanded) */}
        {!isCollapsed && (
          <SidebarGroup>
            <SidebarGroupLabel>Your Skills</SidebarGroupLabel>
            <SidebarGroupContent>
              <div className="px-2 py-1">
                <div className="flex flex-wrap gap-1">
                  {user.languages.slice(0, 6).map((lang: string) => (
                    <Badge 
                      key={lang} 
                      variant="secondary" 
                      className="text-xs bg-purple-100 text-purple-700 border-0 hover:bg-purple-200"
                    >
                      {lang}
                    </Badge>
                  ))}
